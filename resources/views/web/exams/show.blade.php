            ...
            <div class="row">
                <div class="col-md-10 col-md-offset-1 text-center">
                    <ul class="hero-area-tree">
                        ...
                        <li><a
                                href="{{ url('categories/show' . $exam->skill->category->id) }}">{{ $exam->skill->category->name() }}</a>
                        </li>
                    ...
                    <div>
                        @if ($canViewStartBtn)
                            <form method="POST" action="{{ url("exams/start/$exam->id") }}">
                                @csrf
                                <button type="submit" href="{{ url("exams/questions/$exam->id") }}"
                                    class="main-button icon-button pull-left">{{ __('web.startExamBtn') }}
                                </button>
                            </form>
                        @endif
